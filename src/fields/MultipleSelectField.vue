          ...
          <template v-else-if="valueOptions.length">
            <button
              ...
              :class="[
                ...
                {
                  'select-field__select-dropdown-item--active': modelValue.some(
                    el => el === option.value,
                  ),
                },
              ]"
...
const title = computed(() => {
  ...
  props.valueOptions.forEach(({ value, label }) => {
    const item = props.modelValue.find(el => el === value)
    if (item) picked.push(label)
  })
...
const select = (value: string | number) => {
  ...
  const isAlreadyPicked = props.modelValue.includes(value)

  if (isAlreadyPicked && props.modelValue.length < 2) return
